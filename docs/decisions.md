...
| Quick Dictation | Enabled; ready window of 10 minutes (default), 20 minutes, or until the app is closed; cleared when backgrounded | Reduces app switching while vocaphone is running, without claiming background-audio eligibility. The Live Activity's stop button pauses the current window only; the next launch arms a new one. Backgrounding clears standby the same way; Control Center and alerts do not. Installs that arrive with it already off — which an older build could do on one tap — are asked once on Home, never switched back on silently |
