...
internal enum class SetupPage(val title: String, val detail: String, private val step: SetupStep?) {
    WELCOME("Your words, your way", "Speak naturally in the language that feels right.", null),
    ...
    companion object {
        fun resume(status: SetupStatus): SetupPage = entries.drop(1).firstOrNull { !it.isSatisfied(status) } ?: READY
    }
}
