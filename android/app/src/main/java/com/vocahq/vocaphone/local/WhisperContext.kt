...
internal object WhisperCpuConfig {
     ...
     *
     * Raised from 448 after "hi" came back as "Hi. Hi.": 448 was measured on the
     * tiny and base models, and the same window makes a small model repeat often
     * enough to matter. [WhisperCpuConfig.whisperAudioContext] is only asked for
     * at all by the models where the crop was verified — see
     * [LocalModelDescriptor.cropsAudioContext].
     */
     ...
     * wrong — so this asks for twice as much context as the audio needs, and
     * never less than [MINIMUM_AUDIO_CONTEXT]. Past fifteen seconds the full
     * window is the smaller ask, and long recordings whisper already splits into
     * thirty-second windows are left exactly as they were.
     *
     * Sensitivity to a cropped window scales with model size, which is why the
     * medium and large models never ask for one at all: they are the ones that
     * turned a one-word dictation into the same word twice.
     */
